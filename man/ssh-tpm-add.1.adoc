...
*-c*::
  ...
  *ssh-tpm-agent*(1) via *SSH_ASKPASS*. Successful confirmation is signaled
  by a zero exit status from the askpass program, rather than text entered
  into the requester. This mirrors *ssh-add*(1) *-c*.
